# st7789.py — Central B / LAB-ARTE
#
# Versión compacta: solo lo que usa Central B.
# Sin write()/Comfortaa, sin polygon(), sin bitmap(), sin line().
# Eso libera ~4-6 KB de bytecode y permite que el FrameBuffer
# de 64800 bytes (240×135×2) entre sin MemoryError.
#
# Si un nodo necesita Comfortaa, carga st7789_full.py en su lugar.
#
# API disponible:
#   display.print()       — texto con cursor automático
#   display.draw_text()   — texto en coordenadas fijas
#   display.header()      — barra de cabecera con 2 o 3 zonas
#   display.cursor()      — mover / resetear cursor interno
#   display.show()        — volcar framebuffer a pantalla
#   display.fill()        — rellenar pantalla directamente
#   display.fill_rect()   — rectángulo relleno directo
#   display.hline()       — línea horizontal
#   display.vline()       — línea vertical
#   display.rect()        — rectángulo hueco
#   display.blit_buffer() — imagen raw en pantalla
#   display.pixel()       — un píxel en pantalla
#   display.invert()      — invertir colores
#   display.sleep()       — modo sleep
#   display.backlight()   — control de retroiluminación
#   display.fb            — FrameBuffer para dibujar antes de show()
...
class Display:
    """
    Driver ST7789 compacto para Central B.
    ...
    # ── Rellenos directos (sin framebuffer) ───────────────
    def fill(self, color):
        self._set_window(0, 0, self.width - 1, self.height - 1)
    ...
    def draw_text(self, txt, x, y, color,
        ...
        if line_h is None: line_h = 8 * scale + 2
        ...
        for linea in lineas:
            ...
            if scale == 1:
                ...
                # Escala > 1: renderizar carácter a carácter en tiny buffer
                    ...
                    for row in range(8):
                        for col in range(8):
                            ...
                            for sy in range(scale):
                                ...
                                for sx_off in range(scale):
                                    dx = cur_x + col * scale + sx_off
                                    if dx < self.width:
