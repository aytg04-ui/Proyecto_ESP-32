# Central B — Actualizado al st7789 unificado
#
# Cambios vs versión anterior:
#   - rotation=1 añadido al constructor del display
#   - display.init() eliminado (el nuevo driver inicializa solo)
#   - Todo lo demás idéntico: print(), header(), show(), menús, ESP-NOW
...
# ── Pantalla ──────────────────────────────────────────────
# CAMBIO: añadido rotation=1, eliminado display.init()
...
# Broadcast para reenvío al master del compañero
...
def enviar_comando(accion, nombre):
    if accion == "Solicitar medicion":
        ...
        for n,info in nodos.items():
            e.send(info["mac"], b'DORMIR'); nodos[n]["dormido"]=True
    elif accion == "Activar":
        display.fb.fill(st7789.BLACK)
...
# ── Traducción y reenvío al master del compañero ─────────
# Convierte el mensaje de texto plano de tus nodos al formato
# JSON que entiende el servidor del compañero, y lo reenvía
# por broadcast para que el master lo recoja.

def _traducir_a_payload(datos):
    """Convierte el dict interno de datos al formato pl:[{t,v}] del servidor."""
    pl = []
    if datos.get("mpu"):
        ax, ay, az = datos["mpu"]
        pl.append({"t": "AccX", "v": ax})
        pl.append({"t": "AccY", "v": ay})
        pl.append({"t": "AccZ", "v": az})
    if datos.get("gy"):
        amb, obj = datos["gy"]
        pl.append({"t": "TempAmb", "v": round(amb, 1)})
        pl.append({"t": "TempObj", "v": round(obj, 1)})
    if datos.get("dht"):
        t, h = datos["dht"]
        pl.append({"t": "Temp", "v": t})
        pl.append({"t": "Hum",  "v": h})
    if datos.get("mq"):
        v, pct = datos["mq"]
...
def reenviar_al_servidor(origen, datos):
    """
    Traduce los datos de un nodo al formato JSON del compañero
    y los reenvía por broadcast para que el master los recoja.
    """
    try:
        pl = _traducir_a_payload(datos)
        if not pl:
            return
        pkt = json.dumps({
            "type": "FB",
            "id"  : "SLAVE_{}".format(origen),
            "par" : "MASTER_TTGO_GATEWAY",
            "pl"  : pl,
        })
        # El paquete ESP-NOW tiene límite de 250 bytes
        if len(pkt) > 248:
            pkt = json.dumps({
                "type": "FB",
                "id"  : "SLAVE_{}".format(origen),
                "par" : "MASTER_TTGO_GATEWAY",
                "pl"  : pl[:4],   # recortar si es demasiado largo
            })
        e.send(BROADCAST_MAC, pkt.encode())
        print("[SERVER] Reenviado nodo", origen, len(pkt), "bytes")
        ...
        try:
            ...
            if origen == "ACK":
                ...
                if datos:
                    ...
                    pantalla_sucia=True
                    reenviar_al_servidor(origen, datos)  # → master compañero
