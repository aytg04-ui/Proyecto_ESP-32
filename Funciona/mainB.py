...
import network, espnow, ubinascii, time, json
import st7789
...
# [NUEVO] Broadcast para reenvío al master
...
# Estado de activación no bloqueante
activacion = {
    "activa":    False,   # hay una activación en curso
    "nodo":      None,    # nombre del nodo que se está activando ("Todos" = todos)
    "inicio":    0,       # ticks_ms cuando comenzó
    "ultimo_tx": 0,       # último envío de ACTIVAR
    "pendientes": set(),  # nodos que aún no han respondido ACK
}
ACTIVAR_DURACION_MS  = 35000   # tiempo máximo esperando ACK
ACTIVAR_INTERVALO_MS =   400   # cada cuánto reenviar ACTIVAR
...
def enviar_comando(accion, nombre):
    if accion == "Solicitar medicion":
        ...
        # Inicia activación no bloqueante para un nodo
        activacion["activa"]    = True
        activacion["nodo"]      = nombre
        activacion["inicio"]    = time.ticks_ms()
        activacion["ultimo_tx"] = 0
        activacion["pendientes"] = {nombre}
        # Primer envío inmediato
        try:
            e.send(nodos[nombre]["mac"], b'ACTIVAR')
        except OSError:
            pass
        activacion["ultimo_tx"] = time.ticks_ms()
        _pantalla_activando()
    elif accion == "Activar todos":
        # Inicia activación no bloqueante para todos los nodos dormidos
        dormidos = {n for n,info in nodos.items() if info["dormido"]}
        if not dormidos: return
        activacion["activa"]    = True
        activacion["nodo"]      = "Todos"
        activacion["inicio"]    = time.ticks_ms()
        activacion["ultimo_tx"] = 0
        activacion["pendientes"] = dormidos
        for n in dormidos:
            try:
                e.send(nodos[n]["mac"], b'ACTIVAR')
            except OSError:
                pass
        activacion["ultimo_tx"] = time.ticks_ms()
        _pantalla_activando()

def _pantalla_activando():
    nombre = activacion["nodo"]
    pendientes = activacion["pendientes"]
    transcurrido = time.ticks_diff(time.ticks_ms(), activacion["inicio"])
    seg_rest = max(0, (ACTIVAR_DURACION_MS - transcurrido) // 1000)
    display.fb.fill(st7789.BLACK)
    display.cursor(x=0, y=30)
    if nombre == "Todos":
    ...
    display.print("Esperando {}s".format(seg_rest), st7789.WHITE, align="center", max_w=display.width)
    # Mostrar estado de cada nodo pendiente
    for n in sorted(pendientes):
        display.print("Nodo {} ZZZ".format(n), st7789.YELLOW)
    display.show()

def _tick_activacion():
    """Llamar en cada iteración del bucle principal cuando activacion["activa"] es True."""
    global modo
    ahora = time.ticks_ms()

    # Reenviar ACTIVAR periódicamente a los nodos pendientes
    if time.ticks_diff(ahora, activacion["ultimo_tx"]) >= ACTIVAR_INTERVALO_MS:
        for n in list(activacion["pendientes"]):
            try:
                e.send(nodos[n]["mac"], b'ACTIVAR')
            except OSError:
                pass  # nodo en deep sleep, reintentar en el siguiente tick
        activacion["ultimo_tx"] = ahora
        _pantalla_activando()

    # Comprobar si ya no quedan pendientes o se agotó el tiempo
    terminado = (not activacion["pendientes"] or
                 time.ticks_diff(ahora, activacion["inicio"]) >= ACTIVAR_DURACION_MS)
    if terminado:
        # Marcar como no dormidos incluso sin ACK (comando ya fue enviado)
        for n in list(activacion["pendientes"]):
            nodos[n]["dormido"] = False
        activacion["activa"]    = False
        activacion["pendientes"] = set()
        modo = MODO_MONITOR
        global pantalla_sucia
        pantalla_sucia = True

# [NUEVO] Reenvío al servidor del compañero
def _reenviar(origen, datos):
    try:
        pl = []
        if datos.get("mpu"):
            ax,ay,az = datos["mpu"]
            pl += [{"t":"AccX","v":ax},{"t":"AccY","v":ay},{"t":"AccZ","v":az}]
        if datos.get("gy"):
            amb,obj = datos["gy"]
            pl += [{"t":"TempAmb","v":round(amb,1)},{"t":"TempObj","v":round(obj,1)}]
        if datos.get("dht"):
            t,h = datos["dht"]
            pl += [{"t":"Temp","v":t},{"t":"Hum","v":h}]
        if datos.get("mq"):
            v,pct = datos["mq"]
            pl.append({"t":"MQ135","v":v})
        if not pl: return
        pkt = json.dumps({"type":"FB","id":"SLAVE_"+origen,"par":"MASTER_TTGO_GATEWAY","pl":pl})
        if len(pkt) > 248:
            pkt = json.dumps({"type":"FB","id":"SLAVE_"+origen,"par":"MASTER_TTGO_GATEWAY","pl":pl[:4]})
        e.send(BROADCAST_MAC, pkt.encode())
        print("[SERVER] enviado:", pkt[:60])
            ...
            if origen == "ACK":
                ...
                for n,info in nodos.items():
                    if info["mac"]==sender:
                        if cmd == "ACTIVAR":
                            nodos[n]["dormido"] = False
                            activacion["pendientes"].discard(n)
                            print("ACK:ACTIVAR de", n)
                        else:
                            nodos[n]["dormido"]=(cmd=="DORMIR")
                        break
            elif origen == "DORMIDO":
                for n,info in nodos.items():
                ...
                if datos:
                    ...
                    pantalla_sucia=True
                    _reenviar(origen, datos)  # [NUEVO] → master compañero
                    if modo==MODO_DETALLE and nodo_detalle==origen: pantalla_detalle(origen)
        except Exception as err:
            print("Error:",err)

    if btn_confirmar.presionado():
        if activacion["activa"]: pass  # ignorar botones durante activación
        elif modo==MODO_MONITOR:
            ...
            if accion=="Volver":
                ...
                enviar_comando(accion,nodo_activo)
                if accion not in ("Activar","Activar todos"):
                    modo=MODO_MONITOR; pantalla_sucia=True
        elif modo==MODO_DETALLE:
            modo=MODO_MONITOR; pantalla_sucia=True

    if btn_navegar.presionado():
        if activacion["activa"]: pass  # ignorar durante activación
        elif modo==MODO_MONITOR:
            visibles=get_nodos_visibles()
