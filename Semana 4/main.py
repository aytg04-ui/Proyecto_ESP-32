...
MAC_F = ubinascii.unhexlify('ac15186f84a0')
...
# --- Estado de nodos ---
# CORRECCIÓN: "F" usaba MAC_E, ahora usa MAC_F
nodos = {
    ...
    "F": {"datos": None, "ultimo": 0, "activo": False, "mac": MAC_F, "dormido": False},
}
...
ACTIVAR_DURACION_MS  = 35000
ACTIVAR_INTERVALO_MS =   400
...
def pantalla_detalle(nombre):
    ...
    if not datos:
        ...
        if datos.get("mq"):
            ...
            cal, c2 = (("Limpio",    st7789.GREEN)  if v < 1200 else
                       ...
                       ("Malo",      st7789.RED)    if v < 2500 else
                       ("Peligroso", st7789.RED))
            display.print("MQ-135", st7789.CYAN)
            display.print(("ADC:{}  ".format(v),  st7789.WHITE),
                          ("{:.1f}%".format(pct), st7789.WHITE))
    ...
    display.print("Vuelve en {}s".format(max(0, restante)), st7789.WHITE, x=5, y=68)
...
def enviar_comando(accion, nombre):
    if accion == "Solicitar medicion":
        ...
        try: e.send(nodos[nombre]["mac"], b'ACTIVAR')
        except OSError: pass
        ...
        for n in dormidos:
            try: e.send(nodos[n]["mac"], b'ACTIVAR')
            except OSError: pass
        activacion["ultimo_tx"] = time.ticks_ms()
        _pantalla_activando()
...
def _reenviar(origen, datos):
    try:
        ...
        if not pl: return
        pkt = json.dumps({"type": "FB", "id": "SLAVE_" + origen,
                          "par": "MASTER_TTGO_GATEWAY", "pl": pl})
        if len(pkt) > 248:
            pkt = json.dumps({"type": "FB", "id": "SLAVE_" + origen,
                              "par": "MASTER_TTGO_GATEWAY", "pl": pl[:4]})
