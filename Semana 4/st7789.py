...
class Display:
    ...
    def rotation(self, r):
        ...
        self._band_h = min(BAND_H, self.height)
        self._buf = bytearray(self.width * self._band_h * 2)
        self.fb   = framebuf.FrameBuffer(self._buf, self.width, self._band_h, framebuf.RGB565)
        self._cmds = []
    ...
    def _replay_band(self, y_off):
        bh = self._band_h
        self.fb.fill(self._bg_color)
        for cmd in self._cmds:
            ...
            if t == 'fill':
                ...
                _, x, y, w, h, color = cmd
                ry = y - y_off
                if ry + h <= 0 or ry >= bh: continue
                clip_y = max(ry, 0)
                clip_h = min(ry + h, bh) - clip_y
                self.fb.fill_rect(x, clip_y, w, clip_h, color)
            elif t == 'text':
                _, txt, x, y, color = cmd
                ry = y - y_off
                if ry + 8 <= 0 or ry >= bh: continue
                self.fb.text(txt, x, ry, color)
            elif t == 'textS':
                _, txt, x, y, color, scale, bg = cmd
                ry = y - y_off
                if ry + 8*scale <= 0 or ry >= bh: continue
                tiny_buf = bytearray(128)
                tiny_fb  = framebuf.FrameBuffer(tiny_buf, 8, 8, framebuf.RGB565)
                cur_x = x
                for ch in txt:
                    if cur_x + 8*scale > self.width: break
                    tiny_fb.fill(bg)
                ...
                ry = y - y_off
                if 0 <= ry < bh:
                    self.fb.pixel(x, ry, color)

    def show(self):
        bh = self._band_h
        for y0 in range(0, self.height, bh):
            y1 = min(y0 + bh - 1, self.height - 1)
            self._replay_band(y0)
            ...
            if rows == bh:
                ...
                self.spi.write(memoryview(self._buf)[:rows * self.width * 2])
            if self.cs: self.cs.on()
    ...
    def fb_fill(self, color):
        ...
        self._cmds = [('fill', color)]

    def fb_fill_rect(self, x, y, w, h, color):
        self._cmds.append(('fill_rect', x, y, w, h, color))

    def fb_text(self, txt, x, y, color):
        self._cmds.append(('text', txt, x, y, color))

    def fb_pixel(self, x, y, color):
        self._cmds.append(('pixel', x, y, color))
    ...
    def draw_text(self, txt, x, y, color,
        ...
        for linea in lineas:
            if align == "center":
                ...
                px = x
            if scale == 1:
                self._cmds.append(('text', linea[:max_chars], px, cur_y, color))
            else:
                self._cmds.append(('textS', linea[:max_chars], px, cur_y, color, scale, bg))
            cur_y += line_h
        return cur_y
